...
namespace App\Clockwork\Support\Vanilla;
...
class ClockworkMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($response = $this->handleApiRequest($request)) {
            return $response;
        }
        if ($response = $this->handleWebRequest($request)) {
            return $response;
        }
    ...
    {
        $path = is_string($this->clockwork->getConfig()['web']['enable']) ? $this->clockwork->getConfig(
        )['web']['enable'] : '/clockwork';
    ...
    protected function prepareResponse()
    {
        if (!$this->responseFactory && !class_exists(Psr17Factory::class)) {
            throw new \Exception(
                'The Clockwork vanilla middleware requires a response factory or the php-http/discovery package to be installed.'
            );
        }
